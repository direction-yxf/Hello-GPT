...
### 1.1.2 训练范式
&emsp;&emsp;实现模型架构之后，需要一个强大的trainer来训练模型架构中的参数权重，基本上来说不同模型架构训练流程基本一致，并且不论是预训练、指令微调
还是强化学习本质都是一个trainer进行参数权重的迭代更新，因此我们在复现别人的超大模型一般只需修改上述架构代码，训练流程核心不会变。大模型的本质就是神经网络
